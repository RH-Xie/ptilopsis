...
class Tarot:
    ...
    async def onetime_divine(self) -> MessageSegment:
        ...
        flag, pic, desc = await self._get_text_and_image(theme, card_info_list[0])
    ...
    async def _get_text_and_image(self,
                                  ...
                                  ) -> Tuple[bool, MessageSegment, MessageSegment]:
        '''
        ...
        name_en: str = card_info.get("name_en")
        prefix: str = '您的塔罗牌是：\n'
        predesc : str = '✨ 其释义为：'
        if random.random() < 0.5:
            # 正位
            meaning: str = card_info.get("meaning").get("up")
            res = f"🎆 {prefix}『正位』的『{name_cn}』({name_en})\n"
            res += f"{predesc}{meaning}"
            msg = MessageSegment.text(res)
        else:
            meaning: str = card_info.get("meaning").get("down")
            res = f"🎇 {prefix}『逆位』的『{name_cn}』({name_en})\n"
            res += f"{predesc}{meaning}"
            msg = MessageSegment.text(res)
            img = img.rotate(180)
        ...
        return True, MessageSegment.image(raw={"data": img.tobytes(), "mime": "image/png"}), msg
